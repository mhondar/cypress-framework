...
//Caso de prueba 7
    it('Llenamos nuestro primer formulario utilizando data', function(){
        ...
        cy.get('.react-datepicker__month-select').should('be.visible').select(this.datos.fecha[0])

        cy.get('.react-datepicker__year-select').should('be.visible').select(this.datos.fecha[1])

        cy.get('.react-datepicker__day--0' + this.datos.fecha[2]).should('be.visible').click()
        cy.get('#dateOfBirthInput')
        .should('contain.value',this.datos.fecha[0].substring(0,3))
        .should('contain.value',this.datos.fecha[1])
        .should('contain.value',this.datos.fecha[2])
        ...
        .should('have.text', this.datos.fecha[2] + ' ' + this.datos.fecha[0] + ',' + this.datos.fecha[1])
